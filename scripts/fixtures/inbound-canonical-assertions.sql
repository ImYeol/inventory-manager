...
begin
  ...
  -- move-all identity-stable retry: an identical target plan preserves IDs, timestamps, and audit count.
  select jsonb_agg(jsonb_build_object('id',id,'updated_at',updated_at) order by id) into allocation_snapshot from public.factory_arrival_allocations where factory_arrival_id=arrival_id;
  select count(*) into audit_before from public.factory_arrival_allocation_audits where factory_arrival_id=arrival_id;
  no_op:=public.move_factory_arrival_remainders_to_warehouse(jsonb_build_object('arrival_id',arrival_id,'warehouse_id',131,'reason','identical move retry'));
  if coalesce(no_op->>'unchanged','false')<>'true' or (select jsonb_agg(jsonb_build_object('id',id,'updated_at',updated_at) order by id) from public.factory_arrival_allocations where factory_arrival_id=arrival_id)<>allocation_snapshot or (select count(*) from public.factory_arrival_allocation_audits where factory_arrival_id=arrival_id)<>audit_before then raise exception 'move-all identity-stable retry failed'; end if;
  ...
  if exists(select 1 from public.factory_arrival_items where factory_arrival_id=child_arrival and received_quantity<>0) or (select status from public.factory_arrivals where id=child_arrival)<>'READY' or (select sum(allocated_quantity-normally_received_quantity-shortage_closed_quantity) from public.factory_arrival_allocations where factory_arrival_id=child_arrival)<>1 then raise exception 'follow-up correction parity failed'; end if;
  -- corrected follow-up child lifecycle: the open child owns that expected unit, so another child cannot duplicate incoming.
  select count(*) into child_count from public.factory_arrivals where user_id=auth.uid() and follow_up_parent_arrival_id=arrival_id;
  begin perform public.record_factory_arrival_follow_up(jsonb_build_object('closure_id',closure_id,'warehouse_id',131,'quantity',1,'reason','duplicate late carton','receipt_request_id','step9-follow-up-2','receipt_business_date','2026-07-22')); exception when others then failed:=sqlerrm=format('operation_error:closure:%s:Follow-up child is still open.',closure_id); end;
  if not failed or (select count(*) from public.factory_arrivals where user_id=auth.uid() and follow_up_parent_arrival_id=arrival_id)<>child_count or (select sum(allocated_quantity-normally_received_quantity-shortage_closed_quantity) from public.factory_arrival_allocations where factory_arrival_id=child_arrival)<>1 then raise exception 'corrected follow-up child lifecycle duplicated expected incoming'; end if; failed:=false;
