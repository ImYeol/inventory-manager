...
export async function getFactoryArrivalsData(): Promise<FactoryArrivalsDataResult> {
  ...
  const receiptEvents = ensure(receiptEventsRes.data as Array<{ id: number; factory_arrival_id: number; receipt_business_date: string }> | null, receiptEventsRes.error)
