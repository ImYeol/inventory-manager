      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">
            {configured ? '현재 사용자 계정에 저장된 배송 연동 정보입니다.' : '아직 이 사용자에게 저장된 연동 정보가 없습니다.'}
          </p>
        </div>
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-4">
        ...
        <p className="mt-1 text-sm leading-6 text-slate-600">
          새 값을 입력한 항목만 교체되며, 비워 둔 입력칸은 기존 저장 값을 유지합니다.
        </p>
      </div>
        ...
        <div className="space-y-4">
          <SummaryCard
            ...
            lines={[
              {
                label: 'Client ID',
                value: currentSummary.naver.masked.clientId ?? '저장된 키 없음',
              },
            ]}
          ...
          <form className="surface p-5" onSubmit={handleNaverSave}>
            <div className="space-y-1">
              <h2 className="text-lg font-semibold text-slate-950">네이버 키 업데이트</h2>
              <p className="text-sm leading-6 text-slate-500">
                기존 값은 노출되지 않습니다. 새 키를 입력하면 해당 사용자 계정의 저장값을 교체합니다.
              </p>
            </div>
            ...
            {naverError ? (
              <p className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700" aria-live="polite">
                {naverError}
              </p>
            ) : null}
            {naverMessage ? (
              <p className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700" aria-live="polite">
                {naverMessage}
              </p>
            ) : null}

            <div className="mt-5 flex items-center justify-between gap-3">
              <p className={ui.helpText}>저장 후 마스킹된 요약이 바로 갱신됩니다.</p>
        ...
        <div className="space-y-4">
          <SummaryCard
            ...
            lines={[
              {
                label: 'Access Key',
                value: currentSummary.coupang.masked.accessKey ?? '저장된 키 없음',
              },
              {
                label: 'Vendor ID',
                value: currentSummary.coupang.masked.vendorId ?? '저장된 키 없음',
              },
            ]}
          ...
          <form className="surface p-5" onSubmit={handleCoupangSave}>
            <div className="space-y-1">
              <h2 className="text-lg font-semibold text-slate-950">쿠팡 키 업데이트</h2>
              <p className="text-sm leading-6 text-slate-500">
                Access Key, Secret Key, Vendor ID를 함께 입력해야 저장됩니다.
              </p>
            </div>
            ...
            <div className="mt-5 flex items-center justify-between gap-3">
              <p className={ui.helpText}>쿠팡 저장 후 운송장 화면에서 즉시 주문 조회가 가능합니다.</p>
