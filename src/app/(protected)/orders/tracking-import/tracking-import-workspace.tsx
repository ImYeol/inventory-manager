...
import { finalizeTrackingImport, previewTrackingImport, type TrackingPreviewRow } from '@/lib/actions/tracking-import'
...
export default function TrackingImportWorkspace() {
  ...
  return <div className="space-y-4">
    ...
    <TableSurface toolbar={<div className="flex w-full items-center justify-between gap-2"><div className="flex items-center gap-2"><span className="text-sm font-medium">분류 미리보기</span>{result ? <span role="status" className="text-xs text-[color:var(--muted-foreground)]">{result}</span> : null}</div><div className="flex gap-2"><button type="button" className="ui-button-secondary" disabled={isPending || !filename || !rows.some((row) => row.trackingNumber)} onClick={validateRows}>검증</button><button type="button" className="ui-button-primary" disabled={isPending || !(preview ?? []).some((row) => row.dispatchable)} onClick={dispatchRows}>발송</button></div></div>}>
