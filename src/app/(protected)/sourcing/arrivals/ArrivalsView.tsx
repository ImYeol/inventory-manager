...
type Sheet = { kind: 'arrival'; arrivalId: number; operation: Operation } | { kind: 'manual' } | { kind: 'import' } | null
...
export default function ArrivalsView({ schemaState, factories, warehouses, models, arrivals, inboundTemplates = [], productVariants = [] }: { schemaState: SourcingSchemaState; factories: FactoryLookup[]; warehouses: WarehouseLookup[]; models: ModelLookup[]; arrivals: ArrivalRecord[]; inboundTemplates?: InboundTemplateOption[]; productVariants?: Array<{ id: number; label: string }> }) {
  ...
  const saveAllocation = (arrival: ArrivalRecord, item: ArrivalRecord['items'][number]) => { const reason = reasons[item.id]?.trim() ?? ''; if (!reason) return failOperation(new Error('배정 변경 사유를 입력해주세요.'), '', `item-${item.id}`); startTransition(async () => { try { await replaceFactoryArrivalAllocations({ arrivalId: arrival.id, itemId: item.id, reason, allocations: Object.entries(allocationDrafts[item.id] ?? {}).filter(([, quantity]) => quantity > 0).map(([id, quantity]) => ({ warehouseId: Number(id), quantity })) }); succeed('창고 배정을 저장했습니다.') } catch (error) { failOperation(error, '창고 배정에 실패했습니다.', `item-${item.id}`) } }) }
  const moveAll = (arrival: ArrivalRecord) => { const reason = reasons[arrival.id]?.trim() ?? ''; if (!warehouseId) return failOperation(new Error('기본 창고를 선택해주세요.'), '', `arrival-${arrival.id}`); if (!reason) return failOperation(new Error('전체 이동 사유를 입력해주세요.'), '', `arrival-${arrival.id}`); startTransition(async () => { try { await moveFactoryArrivalRemaindersToWarehouse({ arrivalId: arrival.id, warehouseId, reason }); succeed('남은 수량을 기본 창고로 이동했습니다.') } catch (error) { failOperation(error, '남은 수량 이동에 실패했습니다.', `arrival-${arrival.id}`) } }) }
  const closeShortage = (id: number) => { const draft = shortages[id] ?? { quantity: 0, reason: '' }; if (draft.quantity <= 0 || !draft.reason.trim()) return failOperation(new Error('부족 수량과 사유를 입력해주세요.'), '', `allocation-${id}`); startTransition(async () => { try { await closeFactoryArrivalShortage({ allocationId: id, ...draft }); succeed('부족 수량을 종료했습니다.') } catch (error) { failOperation(error, '부족 종료에 실패했습니다.', `allocation-${id}`) } }) }
  const followUp = (id: number) => { const draft = followUps[id] ?? { warehouseId: warehouses[0]?.id ?? 0, quantity: 0, reason: '', receiptBusinessDate: koreaLocalDate() }; if (!draft.warehouseId || draft.quantity <= 0 || !draft.reason.trim() || !draft.receiptBusinessDate) return failOperation(new Error('후속 업무일, 창고, 수량과 사유를 입력해주세요.'), '', `closure-${id}`); startTransition(async () => { try { const receiptRequestId = followUpIds.current[id] ?? requestId(); followUpIds.current[id] = receiptRequestId; await recordFactoryArrivalFollowUp({ closureId: id, ...draft, receiptRequestId }); delete followUpIds.current[id]; succeed('후속 입고를 기록했습니다.') } catch (error) { failOperation(error, '후속 입고에 실패했습니다.', `closure-${id}`) } }) }
  const correct = (id: number) => { const reason = corrections[id]?.trim() ?? ''; if (!reason) return failOperation(new Error('정정 사유를 입력해주세요.'), '', `receipt-line-${id}`); startTransition(async () => { try { const correctionRequestId = correctionIds.current[id] ?? requestId(); correctionIds.current[id] = correctionRequestId; await reverseFactoryReceiptLine({ receiptLineId: id, correctionRequestId, reason }); delete correctionIds.current[id]; succeed('입고 기록을 정정했습니다.') } catch (error) { failOperation(error, '입고 정정에 실패했습니다.', `receipt-line-${id}`) } }) }
  ...
  const renderManual = () => <section className="space-y-4">{sheetError ? <p role="alert" aria-live="assertive" className="text-sm font-medium text-[color:var(--warning-foreground)]">{sheetError}</p> : null}<div className="grid gap-3 md:grid-cols-2"><FieldSelect label="공장" value={factoryId} options={factoryOptions} onValueChange={setFactoryId} disabled={!factoryOptions.length} /><FieldSelect label="입고 예정 창고" value={warehouseId} options={warehouses} onValueChange={setWarehouseId} disabled={!warehouses.length} /><label><span className={ui.label}>예정 입고일</span><Input aria-label="예정 입고일" type="date" value={expectedDate} onChange={(event) => setExpectedDate(event.target.value)} /></label></div><div className="space-y-3"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold text-[color:var(--foreground)]">등록 항목</h2><Button type="button" variant="outline" size="sm" onClick={() => setRows((current) => [...current, createRow()])}>행 추가</Button></div>{rows.map((row, index) => <div key={row.key} className="grid gap-2 border-t border-[color:var(--border)] pt-3 md:grid-cols-[1fr_8rem]"><label><span className={ui.label}>항목 #{index + 1} 상품 옵션</span><ProductVariantCombobox aria-label={`항목 #${index + 1} 상품 옵션`} variants={variants} value={row.modelId ? `${row.modelId}:${row.sizeId}:${row.colorId}` : null} onValueChange={(id) => { const variant = variants.find((item) => item.id === id); setRows((current) => current.map((item) => item.key === row.key ? { ...item, modelId: variant?.modelId ?? '', sizeId: variant?.sizeId ?? '', colorId: variant?.colorId ?? '' } : item)) }} /></label><Input aria-label={`항목 #${index + 1} 수량`} placeholder="수량" type="number" min={1} value={row.orderedQuantity} onChange={(event) => setRows((current) => current.map((item) => item.key === row.key ? { ...item, orderedQuantity: event.target.value ? Number(event.target.value) : '' } : item))} /></div>)}</div><Button type="button" onClick={submitManual} disabled={isPending || schemaState.status === 'missing'}>예정 입고 등록</Button></section>
  const title = sheet?.kind === 'arrival' ? `${selected?.factoryName ?? ''} 입고 예정` : sheet?.kind === 'manual' ? '수동 입고 예정 추가' : '입고 엑셀 가져오기'
