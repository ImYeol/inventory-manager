    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
            <span>📁 엑셀 업로드</span>
            <input
              type="file"
              accept=".xlsx,.xls"
      ...
      {courierRows.length > 0 && (
        <>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 bg-green-50 border-b border-slate-200">
              <div className="flex items-center gap-2">
                <span className="text-lg font-bold text-green-700">네이버</span>
              ...
              <div className="flex items-center gap-2">
                {naverSentMessage && (
                  <span className="text-sm text-green-600 font-medium">{naverSentMessage}</span>
                )}
                <button
            ...
            {naverError && (
              <div className="px-4 py-3 bg-red-50 text-red-600 text-sm">{naverError}</div>
            )}
              ...
              <>
                      ...
                      {naverOrders.map((o, i) => (
                        <tr
                          ...
                          className={`transition-colors ${
                            o.excluded ? 'bg-slate-50 opacity-50' : o.sent ? 'bg-green-50' : o.failed ? 'bg-red-50' : 'hover:bg-slate-50'
                          }`}
                        >
                          ...
                          <td className="px-4 py-2.5 border-b border-slate-100">
                            {o.trackingNumber ? (
                              <span className="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-bold font-mono">
                                {o.trackingNumber}
                              </span>
                            ) : (
                          ...
                          <td className="px-4 py-2.5 border-b border-slate-100 text-center">
                            {o.sent && <span className="text-green-600 text-xs font-bold">완료</span>}
                ...
                <div className="md:hidden divide-y divide-slate-100">
                  {naverOrders.map((o, i) => (
                    <div
                      key={o.productOrderId}
                      className={`px-4 py-3 ${o.excluded ? 'opacity-50 bg-slate-50' : o.sent ? 'bg-green-50' : o.failed ? 'bg-red-50' : ''}`}
                    >
          ...
          {/* 쿠팡 섹션 */}
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 bg-orange-50 border-b border-slate-200">
              <div className="flex items-center gap-2">
                <span className="text-lg font-bold text-orange-700">쿠팡</span>
                <span className="text-sm text-slate-500">
                  {coupangLoading
                    ? '조회 중...'
              ...
              <div className="flex items-center gap-2">
                {coupangSentMessage && (
                  <span className="text-sm text-green-600 font-medium">{coupangSentMessage}</span>
                )}
                <button
                  onClick={handleCoupangSend}
                  disabled={coupangSending || coupangSendableCount === 0}
                  className="px-4 py-2 bg-orange-600 text-white rounded-lg text-sm font-medium hover:bg-orange-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
            ...
            {coupangError && (
              <div className="px-4 py-3 bg-red-50 text-red-600 text-sm">{coupangError}</div>
            )}
