    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">기간 단위</label>
        ...
        {loading && (
          <div className="mt-2 text-sm text-blue-500">데이터 로딩 중...</div>
        )}
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-200 bg-slate-50">
          <h3 className="text-sm font-semibold text-slate-600">모델별 현재 재고 요약</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50 text-left">
                <th className="px-4 py-2.5 text-xs font-semibold text-slate-500 border-b border-slate-200">순위</th>
                <th className="px-4 py-2.5 text-xs font-semibold text-slate-500 border-b border-slate-200">모델명</th>
                <th className="px-4 py-2.5 text-xs font-semibold text-slate-500 border-b border-slate-200 text-right">총 재고</th>
                <th className="px-4 py-2.5 text-xs font-semibold text-slate-500 border-b border-slate-200">비율</th>
              </tr>
            </thead>
            <tbody>
              {(() => {
                ...
                return initialSummary.map((s, i) => (
                  <tr key={s.modelName} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-2.5 text-sm text-slate-500 border-b border-slate-100">{i + 1}</td>
                    <td className="px-4 py-2.5 text-sm font-medium text-slate-800 border-b border-slate-100">{s.modelName}</td>
                    <td className="px-4 py-2.5 text-sm font-semibold text-slate-800 border-b border-slate-100 text-right">{s.total}</td>
