...
- **입고처 (`supplier`, `Factory`, 테이블 `factories`)**: 입고 출고 리스트 파일을 발행하는 주체다. 공장 자신일 수도, 1688 등 구매 대행을 위한 배대지(포워더)일 수도 있으며 별도 구분 필드는 두지 않는다(ADR-035). 구 명칭 `외부 공장`을 대체하는 UI 용어이며, 코드·DB의 `supplier`/`Factory`/`factories`는 그대로 유지한다.
- **공급자 외부 SKU (`SupplierExternalSku`)**: 특정 입고처가 사용하는 정확한 재고 단위 식별자다. 색상·사이즈가 다르면 다른 외부 SKU이며 상품명 유사도로 연결하지 않는다.
