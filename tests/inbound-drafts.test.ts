...
describe('manual inbound drafts', () => {
  it('only considers a supplier SKU link exact when supplier, template, and external SKU all match', () => {
    expect(inboundSupplierSkuKey({ supplierId: 4, template: 'summer-26', externalSku: 'EXT-001' })).toBe('4:summer-26:EXT-001')
  ...
  it('keeps unmatched rows out of receipt and creates exact links only after receipt', () => {
    ...
    expect(schema).toContain('insert into public.supplier_sku_links')
