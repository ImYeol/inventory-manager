...
| `FileDropInput` | `file-drop-input.tsx` | 파일 선택 canonical drag-and-drop 입력 | `ariaLabel`, `accept`, `onFile`, 선택 `hint`, `description` | 모든 화면의 파일 업로드 필드(샘플 파일, 증빙 파일 등) | bare `<Input type="file">`을 새로 만들지 않는다. 선택된 파일명 표시와 drag-active 상태(`data-drag-active`, `.ui-file-drop`)를 생략하지 않는다. |
| `FilterToolbar` | `filter-toolbar.tsx` | data-surface toolbar의 filter/meta layout | `children`, `className` | `TableSurface`의 toolbar slot 안에서 좌측 filter cluster + 우측 meta cluster를 배치. bordered 박스가 아니라 layout만 담당한다 | 독립 filter 박스로 쓰거나 테이블과 분리된 별도 surface로 띄우지 않는다. |
| `FixedSheet` | `fixed-sheet.tsx` | portal 기반 고정형 입력 overlay | `open`, `title`, `description`, `onClose`, `children`, `className` | 긴 form 또는 viewport 고정 overlay. body scroll lock, Escape/overlay close, 고유 title id와 content-above-overlay layering을 기본으로 가진다. | 짧은 edit flow에는 사용하지 않고, page-local overlay/blur stacking을 재구현하지 않는다. |
| `Input` | `input.tsx` | 공용 text/input control | native `<input>` props | 검색과 text/date/number 입력 | 화면별 input class 조합을 만들지 않는다. |
| `InventoryDataTable`, `InventoryDataRow`, `InventoryColumnKey`, `InventoryStatusVariant` | `inventory-data-table.tsx` | visible-column과 row motion을 갖는 재고 조회 table | `rows`, `visibleColumns`; 행은 model/option/warehouse/quantity/latest inbound·outbound/status와 선택적 `onAdjust` row action | 재고 목록의 canonical 조회 table. 행에 `onAdjust`가 있으면 hover-revealed `조정` 버튼을 렌더한다 | 편집형 입력 table로 사용하거나 visible-column 동작을 재구현하지 않는다. |
| `InventoryTableToolbar` | `inventory-table-toolbar.tsx` | 재고 목록의 warehouse/search/status/column/action toolbar | warehouses, selected warehouse, search, status filter, columns, inbound/outbound/adjustment/transfer callbacks | `InventoryDataTable`와 짝인 재고 조회 toolbar. 4개 action은 `SplitButton`(주 동작 `수동 입고` + caret 메뉴의 수동 출고/실사 조정/창고 이동) 하나로 노출한다. 신규 조합의 첫 수량도 `입고` action으로 처리한다. | 별도 `재고 추가` action 또는 다른 도메인의 toolbar를 copy-paste하지 않는다. 4개 동일 크기 버튼을 나열하는 이전 형태로 되돌리지 않는다. |
...
| `ParseTemplateBuilder` | `parse-template-builder.tsx` | 파일 → 시트/헤더 행 선택 → 컬럼-역할 매핑 → 미리보기 공유 파싱 primitive([ADR-034](../adr/0034-parse-template-shared-primitive.md), [ADR-035](../adr/0035-inbound-supplier-is-shipping-list-issuer.md)) | `roles`(역할 스키마), `sample`, `sheetName`/`headerRowNumber`/`mapping`과 각 `onChange`, `presets`/`selectedPresetId`/`onPresetSelect`, `autoMatch` | 입고(`InboundRegistrationSheet`)·주문 송장(`tracking-import-workspace`)의 파일-파싱 화면과 입고처 상세 modal(`FactoriesView`)의 템플릿 관리 | 화면별로 시트/헤더/컬럼 매핑 UI를 다시 구현하지 않는다. 저장 계층(버전 vs 프리셋)은 이 컴포넌트가 아니라 도메인 action이 소유한다. |
...
| `SplitButton` | `split-button.tsx` | 주 동작 1개 + caret 메뉴(부가 동작 cluster)로 구성된 단일 action group | `label`, `onClick`, `menuLabel`, `children`(메뉴에 넣을 `DropdownMenuItem`), `variant`, `size` | 하나의 가장 흔한 동작과 관련 있지만 덜 쓰는 동작 여러 개를 한 toolbar 슬롯으로 묶을 때(예: `InventoryTableToolbar`의 수동 입고 + 수동 출고/실사 조정/창고 이동) | 4개 이상 나열된 동일 크기 버튼을 그대로 두거나, 화면별로 caret 버튼 쌍을 다시 조립하지 않는다. |
