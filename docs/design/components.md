...
이 문서는 Seleccase Inventory의 컴포넌트 재사용을 위한 canonical source of truth([ADR-028](../adr/0028-component-inventory-source-of-truth.md))다. semantic 값은 [tokens.md](./tokens.md), 표 배치는 [data-layouts.md](./data-layouts.md), UI 원칙과 경로 규칙은 [UI_GUIDE.md](./ui-guide.md)의 [Shared Primitive](./ui-guide.md#shared-primitive)를 따른다.
...
| `DataTable`, `DataTableProps` | `data-table.tsx` | `@tanstack/react-table` 기반 canonical 조회 table — 정렬 없이 정적 시트/모달 중첩 table(`bare`)부터 정렬·컬럼 가시성·페이지네이션까지 한 primitive로 표현 | 현재 API에는 `columns`, `rows`, `emptyState`, `bare`, `queryStart`, `queryEnd`, `actionStart`, `actionEnd`, `actionAlignment`(`split`/`start`/`end`), legacy `toolbarStart`/`toolbarEnd`, `pageSizeOptions` 등이 있다 | standalone은 Action Row → Query Row → TableSurface 순서로 렌더하고 Query/Action 각 row를 start/end cluster로 분리한다. action-only row는 명시적 alignment를 쓴다. `bare`는 TableSurface/Overlay 안에서 chrome 없이 쓴다 | 편집 input table이나 화면별 `<table>`을 만들지 않는다. migration 완료 전에는 legacy props를 임의 삭제하지 않는다. |
...
| `FilterToolbar` | `filter-toolbar.tsx` | standalone Query Row/Action Row의 filter/meta layout | `children`, `className`; `QueryRowStart/End`, `ActionRowStart/End`; `ActionRow.align`; `IndependentActionGroup`; `QueryResetButton` | TableSurface 밖 controls 영역. `IndependentActionGroup`은 독립 compact 운영 버튼, `QueryResetButton`은 query 초기화에만 쓴다. | 자체 bordered surface나 page-local toolbar를 새로 만들지 않는다. text controls를 center 정렬하지 않는다. query reset을 ghost로 낮추거나 독립 action을 연결형 ButtonGroup으로 만들지 않는다. |
| `Dialog` / `WorkDialog*` / `Sheet` | `dialog.tsx`, `sheet.tsx`, `modal.tsx` | portal 기반 작업 overlay | `WorkDialog`, `WorkDialogTrigger`, `WorkDialogContent`, `WorkDialogHeader`, `WorkDialogBody`, `WorkDialogFooter`; Title/Description은 canonical Dialog exports 사용 | `WorkDialogContent`는 desktop wide overlay와 mobile full-screen을 제공하며 header/body/footer 구조로 긴 table·form을 완결한다 | 오른쪽 drawer에 긴 table을 억지로 압축하지 않는다. `WorkDialog`에서 Title/Description을 생략하지 않는다. |
...
| `TableSurface` | `table-surface.tsx` | 현재는 toolbar slot과 table을 함께 감쌀 수 있는 surface; 승인 target은 실제 table header/body/footer만 감싸는 surface | 현재 API: `toolbar`, `children`, `footer`, `className`, `scrollClassName` | 현재 소비처 호환을 유지한다. migration 후 standalone controls는 밖에 두고 embedded-bare만 seamless하게 감싼다 | target이 아직 모든 소비처에 구현됐다고 가정하거나, 기존 `toolbar` API를 이 단계에서 삭제하지 않는다. |
...
| 역할 | `size` | 높이 토큰 (SoT: [tokens.md](./tokens.md); 현재 구현: `globals.css`) | 예시 |
| --- | --- | --- | --- |
...
| --- | --- | --- | --- |
| shell/surface | `shell`, `shellNarrow`, `surface`, `surfaceMuted`, `surfaceStrong`, `panel`, `panelHeader`, `panelBody` | `Card`, `FilterToolbar`, `Dialog`, `Sheet`, `Modal` | page layout과 tokenized surface bridge |
...
| data surface | `dataSurface`, `dataScroll`, `dataFooter`, `dataMeta` | `TableSurface`, `FilterToolbar` | controls 밖 table surface의 border/scroll/footer/meta |
...
| modal | `modal`, `modalOverlay`, `modalContent`, `modalHeader`, `modalBody`, `modalFooter` | `Modal`, `Dialog`, `Sheet` | overlay and dialog structure |
...
| 남은 migration target | 현재 역할 | 목표 primitive | 해소 예정 |
| `src/app/(protected)/sourcing/factories/FactoriesView.tsx` 약 579행의 직접 `<Table>` | 입고처 상세 안의 파싱 template/보조 목록 | `DataTable(bare)` | unified-data-layout-system Step 3 |

`FactoriesView.tsx`에는 직접 `<Table>` 예외가 아직 남아 있다. 따라서 모든 hand-roll table이 해소됐다고 주장하지 않는다. `InventoryView`와 삭제된 legacy primitive에 대한 기록은 역사적 맥락이며, 새 구현은 실제 소비처를 먼저 확인한다.
