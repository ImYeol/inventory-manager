...
| `InventoryTableToolbar` | `inventory-table-toolbar.tsx` | 재고 목록의 warehouse/search/status/column/action toolbar | warehouses, selected warehouse, search, status filter, columns, inbound/outbound callbacks | `InventoryDataTable`와 짝인 재고 조회 toolbar. 신규 조합의 첫 수량도 `입고` action으로 처리한다. | 별도 `재고 추가` action 또는 다른 도메인의 toolbar를 copy-paste하지 않는다. |
