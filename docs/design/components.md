...
| `ParseTemplateBuilder` | `parse-template-builder.tsx` | 파일 → 시트/헤더 행 선택 → 컬럼-역할 매핑 → 미리보기 공유 파싱 primitive([ADR-034](../adr/0034-parse-template-shared-primitive.md)) | `roles`(역할 스키마), `sample`, `sheetName`/`headerRowNumber`/`mapping`과 각 `onChange`, `presets`/`selectedPresetId`/`onPresetSelect`, `autoMatch` | 입고(`InboundRegistrationSheet`)·주문 송장(`tracking-import-workspace`)의 파일-파싱 화면과 설정의 파싱 템플릿 관리 | 화면별로 시트/헤더/컬럼 매핑 UI를 다시 구현하지 않는다. 저장 계층(버전 vs 프리셋)은 이 컴포넌트가 아니라 도메인 action이 소유한다. |
